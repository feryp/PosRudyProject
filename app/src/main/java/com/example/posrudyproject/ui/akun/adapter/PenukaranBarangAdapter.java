    ...
    public void onBindViewHolder(@NonNull PenukaranBarangViewHolder holder, int position) {
        ...
        holder.btnPlus.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                editor.putString("totalHarga", String.valueOf(total + Double.valueOf(((item.getHargaBarang().replace(",","")).replace("Rp", "")).replace(".",""))));
                editor.apply();
        ...
        /*holder.checkBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                ...
                if (b) {
                    ...
                    editor.putString("totalHarga",String.valueOf(total + (Double.valueOf(item.getJumlahBarang()) * Double.valueOf(((item.getHargaBarang().replace(",","")).replace("Rp", "")).replace(".","")))));
                } else {
                    holder.btnMinus.setEnabled(false);
                    holder.btnPlus.setEnabled(false);
                    holder.jumlahBarang.setText(item.getJumlahBarang());
                    editor.putBoolean(item.getArtikelBarang()+ "-checkBox", false);
                    if (total.equals(Double.valueOf(((item.getHargaBarang().replace(",","")).replace("Rp", "")).replace(".","")))) {
                        editor.putString("totalHarga", "0.00");
                    } else if (total < Double.valueOf(((item.getHargaBarang().replace(",","")).replace("Rp", "")).replace(".",""))) {
                        editor.putString("totalHarga", "0.00");
                    } else {
                        editor.putString("totalHarga",String.valueOf(total - (Double.valueOf(item.getJumlahBarang()) * Double.valueOf(((item.getHargaBarang().replace(",","")).replace("Rp", "")).replace(".","")))));
                    }
                    System.out.println(total);
                    System.out.println(Double.valueOf(((item.getHargaBarang().replace(",","")).replace("Rp", "")).replace(".","")));
