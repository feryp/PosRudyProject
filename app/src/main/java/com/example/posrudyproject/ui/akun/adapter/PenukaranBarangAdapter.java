...
public class PenukaranBarangAdapter extends RecyclerView.Adapter<PenukaranBarangViewHolder> {
    ...
    public void onBindViewHolder(@NonNull PenukaranBarangViewHolder holder, int position) {
        ...
        //editor.putString("totalHarga", String.valueOf(Double.valueOf(item.getJumlahBarang()) * Double.valueOf((item.getHargaBarang()).replace("Rp",""))));
        editor.apply();

        holder.btnPlus.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Double total = Double.valueOf(preferences.getString("totalHarga", "0.00"));
                ...
                editor.putString("totalHarga", String.valueOf(total + Double.valueOf(((item.getHargaBarang()).replace("Rp", "")))));
                editor.apply();
        ...
        /*holder.checkBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                ...
                if (b) {
                    ...
                    editor.putString("totalHarga",String.valueOf(total + (Double.valueOf(item.getJumlahBarang()) * Double.valueOf(((item.getHargaBarang()).replace("Rp", ""))))));
                } else {
                    holder.btnMinus.setEnabled(false);
                    holder.btnPlus.setEnabled(false);
                    holder.jumlahBarang.setText(item.getJumlahBarang());
                    editor.putBoolean(item.getArtikelBarang()+ "-checkBox", false);
                    if (total.equals(Double.valueOf(((item.getHargaBarang()).replace("Rp", ""))))) {
                        editor.putString("totalHarga", "0.00");
                    } else if (total < Double.valueOf(((item.getHargaBarang()).replace("Rp", "")))) {
                        editor.putString("totalHarga", "0.00");
                    } else {
                        editor.putString("totalHarga",String.valueOf(total - (Double.valueOf(item.getJumlahBarang()) * Double.valueOf(((item.getHargaBarang()).replace("Rp", ""))))));
                    }
                    System.out.println(total);
                    System.out.println(Double.valueOf(((item.getHargaBarang()).replace("Rp", ""))));
